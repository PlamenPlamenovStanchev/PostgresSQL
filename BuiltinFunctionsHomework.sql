...
ADD COLUMN capital_code VARCHAR(10);
UPDATE countries
...
SELECT	
	...
	SUBSTRING('River Information' FROM '/d{1,4}') AS river_lenght
FROM
	view_river_info;
...
SELECT 
    ...
    LENGTH(CAST(population AS TEXT)) AS population_length
FROM 
    countries;
...
SELECT
	peak_name,
	SUBSTRING(peak_name FROM LENGTH(peak_name) - 3 FOR 4) AS positive_right,
