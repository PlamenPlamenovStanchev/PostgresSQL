...
FROM
	customers
WHERE 
	loyalty_card = TRUE AND (last_name LIKE '%m%' OR last_name LIKE '%M%')
ORDER BY
	last_name DESC,
	id ASC;
...
FROM 
    orders
WHERE 
    created_at > '2025-01-01' AND customer_id BETWEEN 15 AND 30
ORDER BY 
...
SELECT
	i.name AS item_name,
	ROUND(AVG(r.rating), 2) AS avarage_rating,
	COUNT(r.customer_id) AS total_reviews,
	b.name AS brand_name,
	c.name AS classificaton_name
FROM
	reviews AS r
JOIN
	items AS i
